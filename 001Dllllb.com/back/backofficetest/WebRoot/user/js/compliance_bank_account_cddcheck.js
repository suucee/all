...
bankAccountList.prototype={	
	   ...
	   getData:function (_pageNo) {
			...
			$.jsonRPC.request("adminCheckBankAccountService.getBankCheckList",{
       ...
       initView:function(result){
			...
			for ( var i = 0; i < list.length; i++) {
				...
				if(obj.path!=null&&obj.path!=""){
				    attachment='&nbsp;&nbsp;<a   href=' +this.urlroot+obj.path + '><i class="icon-download-alt"  stytle="color:#000;"></i>&nbsp;附件</a>';
				}
				html += '<tr><td >' + obj.cddcheckId+ '</td>' 
				...
				+ '	<td>' + (obj.checkTime==null?"--":toDate(obj.checkTime.time)) + '</td>'
				+ '	<td>' + (obj.adminName==null?"--":obj.adminName) + '</td>'
				+ '	<td>'+dohtml+'</td><tr>';
			}
