...
function unbindUser(login) {
	layer.confirm('您确定要解除MT4账号【'+login+'】与其网站账户的绑定吗？', {
		  btn: ['即刻解除','放弃'] //按钮
		},function(){
		...
		success : function(result) {
			if (result != null) {
				...
				if(result.list.list.length==0){
					$("tbody#result_list1").html('<tr><td colspan="7">没有检索到相关内容。</td></tr>');
					return;
				}
				var html = '';
...
//							action = ' <button class="btn btn-a" onclick="javascript:bindUser('+item.login+');">绑定</button>';	
							action = ' <a class="btn btn-a"  onclick="parent.openNewWindow('+"'./../admin/mt4s_bind.html?login="+item.login+"'"+',updateList)">绑定</a>';	
						}
						name += '（未绑定）';
						//，没绑定就不显示下划线
		...
		success : function(result) {
			if (result != null) {
				...
				if (item.user == null) {
					...
					name += '<a  onclick="parent.openNewWindow(\'./../admin/user_detail.html?id='+item.user.id+'\',null)\"><i class="fa fa-user"></i>' + item.user._name + '</a>';
