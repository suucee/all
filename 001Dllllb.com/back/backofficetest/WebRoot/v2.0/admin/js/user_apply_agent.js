		...
		if (result != null) {
			...
			for ( var i in result.list.list) {
				...
				if (loginUser.role == "OperationsManager" || loginUser.role == "ComplianceOfficer") {
					...
					do_html += 'opacity: 0;filter: alpha(opacity=0);cursor: pointer"/><i class="fa fa-cloud-upload" aria-hidden="true"></i></button>';
					do_html += '&nbsp;&nbsp;<button class="btn btn-primary" onclick="sendEmail('+item['attach_id']+')" title="发送协议邮件"><i class="fa fa-envelope-o" aria-hidden="true"></i></button>';
					do_html += '&nbsp;&nbsp;<button class="btn btn-primary"  onclick="goSeeProfile('+item["uId"]+')" title="设置成代理"><i class="fa fa-gears" aria-hidden="true"></i></button> ';
