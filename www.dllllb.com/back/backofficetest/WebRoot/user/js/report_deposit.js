...
function refreshList(pageNo) {
	$.jsonRPC
			.request(
					"adminReportService.getDepositPage",
					{
							...
							if (result != null) {
								...
								for ( var i in result.list.list) {
									...
									html += '<tr>'
											...
											+ '<a href="finance_deposit_list.html?scheme=deposited&userid='+item.uid+'">'+(item.amountSum).toFixed(2)+'</a>'
											+ '</td>'
											+ '<td>'
											+ '<a href="finance_deposit_list.html?scheme=deposited&userid='+item.uid+'">'+item.depositcounts+'</a>'
											+ '</td>'
											+ '<td>'
