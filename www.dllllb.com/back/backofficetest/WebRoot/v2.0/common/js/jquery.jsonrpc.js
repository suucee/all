	...
	$
			.extend({
				jsonRPC : {
					...
					/*
					 * Provides the RPC client with an optional default endpoint
					 * and namespace
					 * 
					 ...
					 * pages not to be cached by the browser. Setting cache to
					 * false also appends a query string parameter,
					...
					setup : function(params) {
						...
						this.cache = params.cache !== undefined ? params.cache
								: true;
						return this;
					},

					/*
					 * Convenience wrapper method to allow you to temporarily
					 * set a config parameter (endPoint or namespace) and ensure
					 * it gets set back to what it was before
					 * 
					 * @param {object} The params object which can contains
					 * endPoint {string} The default endpoint for RPC requests
					 * namespace {string} The default namespace for RPC requests
					 * @param {function} callback The function to call with the
					 * new params in place
					 */
					withOptions : function(params, callback) {
						this._validateConfigParams(params);
						// No point in running if there isn't a callback
						// received to run
					 ...
					 * 
					 * @param {string} method The name of the rpc method to be
					 * called @param {object} options A collection of object
					 * which can contains params {array} the params array to
					 * send along with the request success {function} a function
					 * that will be executed if the request succeeds error
					 * {function} a function that will be executed if the
					 * request fails url {string} the url to send the request to
					 * id {string} the provenance id for this request (defaults
					 * to 1) cache {boolean} If set to false, it will force
					 * requested pages not to be cached by the browser. Setting
					 * cache to false also appends a query string parameter,
					 * "_=[TIMESTAMP]", to the URL. (Default: cache value set
					 * with the setup method) @return {undefined}
					 */
					request : function(method, options) {
						...
						this._validateRequestCallbacks(options.success,
								options.error);

						// Perform the actual request
						this._doRequest(JSON.stringify(this._requestDataObj(
								method, options.params, options.id)), options);

						return true;
					},

					/*
					 * Submits multiple requests Takes an array of objects that
					 * contain a method and params
					 * 
					 * @params {array} requests an array of request object which
					 * can contain method {string} the name of the method param
					 * {object} the params object to be sent with the request id
					 * {string} the provenance id for the request (defaults to
					 * an incrementer starting at 1) @param {object} options A
					 * collection of object which can contains success
					 * {function} a function that will be executed if the
					 * request succeeds error {function} a function that will be
					 * executed if the request fails url {string} the url to
					 * send the request to @return {undefined}
					 */
					batchRequest : function(requests, options) {
						...
						for (var i = 0; i < requests.length; i++) {
							request = requests[i];
							data.push(this._requestDataObj(request.method,
					...
					// Validate request params. Must be a) empty, b) an object
					// (e.g. {}), or c) an array
					...
					_doRequest : function(data, options) {
						...
						$.ajax({
							...
							processData : false,
							error : function(json) {
								_that._requestError.call(_that, json,
										options.error);
							},
					...
					// Determines the appropriate request URL to call for a
					// request
					...
					_requestDataObj : function(method, params, id) {
						var dataObj = {
							...
							method : this.namespace ? this.namespace + '.'
									+ method : method,
							id : id
						}
						if (params !== undefined) {
					...
					_requestError : function(json, error) {
						if (error !== undefined && typeof (error) === 'function') {
							...
							var msg = this._response().error.msg ? this._response().error.msg : this._response().error;
							if (msg == '请先登录，或者您的登录已超时！') {
								window.top.open('../common/login.html', '_self');
							} else if (msg == 'Internal server error') {
								//layer.alert("服务器维护中，请稍后重试！");
							} else {
					...
					_requestSuccess : function(json, success, error) {
						...
						// If we've encountered an error in the response,
						// trigger the error callback if it exists
						if (response.error) {
							if (typeof (error) === 'function') {
								error(response);
							} else {
								var msg = response.error.msg ? response.error.msg : response.error;
						...
						// Otherwise, successful request, run the success
